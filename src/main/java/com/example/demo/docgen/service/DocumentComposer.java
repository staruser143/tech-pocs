...
public class DocumentComposer {
    ...
    public byte[] generateDocument(DocumentGenerationRequest request) {
        ...
        try {
            // Load template structure (resolve base template ids using runtime data)
            DocumentTemplate template = templateLoader.loadTemplate(request.getTemplateId(), request.getData());
